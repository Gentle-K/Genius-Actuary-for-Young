...
export function RolesPage() {
  ...
  const text = {
    eyebrow: isZh ? '管理台' : 'Admin',
    members: isZh ? '成员' : 'members',
    permissionMatrix: isZh ? '权限矩阵' : 'Permission matrix',
    permission: isZh ? '权限项' : 'Permission',
    allowed: isZh ? '允许' : 'Allowed',
    blocked: isZh ? '阻止' : 'Blocked',
    assignmentTitle: isZh ? '用户角色分配' : 'User role assignment',
    assignmentDescription:
      isZh
        ? '直接在当前管理界面切换角色，不必再跳到另一套后台模板。'
        : 'Toggle roles directly from the admin surface without switching to a separate template.',
  }

  return (
    <div className="space-y-6">
      <PageHeader eyebrow={text.eyebrow} title={t('admin.title')} description={t('admin.subtitle')} />
      ...
      <Card className="overflow-hidden p-6">
        <h2 className="mb-4 text-lg font-semibold text-text-primary">{text.permissionMatrix}</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full border-separate border-spacing-y-2">
            <thead>
              <tr>
                <th className="px-4 py-2 text-left text-xs uppercase tracking-[0.18em] text-text-muted">
                  {text.permission}
                </th>
                {roles.map((role) => (
                  ...
                  {roles.map((role, index) => (
                    ...
                    >
                      {role.permissions.includes(permission.id) ? (
                        <Badge tone="success">{text.allowed}</Badge>
                      ) : (
                        <Badge tone="neutral">{text.blocked}</Badge>
                      )}
                    </td>
                  ))}
                </tr>
      ...
      <Card className="space-y-4 p-6">
        <div>
          <h2 className="text-lg font-semibold text-text-primary">{text.assignmentTitle}</h2>
          <p className="mt-1 text-sm leading-7 text-text-secondary">{text.assignmentDescription}</p>
        </div>
